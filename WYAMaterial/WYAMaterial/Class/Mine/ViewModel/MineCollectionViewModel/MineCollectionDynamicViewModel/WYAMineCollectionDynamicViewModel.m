//
//  WYAMineCollectionDynamicViewModel.m
//  WYAMaterial
//
...
@implementation WYAMineCollectionDynamicViewModel
- (NSArray *)testSource {
...
- (WYAMineCollectionDynamicImageModel *)aaa {
    WYAMineCollectionDynamicImageModel * imageModel = [[WYAMineCollectionDynamicImageModel alloc] init];
    imageModel.imageUrl                             = @"";
    return imageModel;
}

- (WYAMineCollectionDynamicCommentsModel *)ccc {
    WYAMineCollectionDynamicCommentsModel * model = [[WYAMineCollectionDynamicCommentsModel alloc] init];
    model.comments                                = @"这是评论,这是及房价地价结构激动机构及东莞为奇偶电视剧佛为加大搜附近偶滴神覅京东就感觉如果今日任务企鹅窝hi去很符合方式";
    model.personName                              = @"xxx";
    return model;
}

- (WYAMineCollectionDynamicModel *)bbbWithNumber:(int)number
                                            name:(NSString *)name
    ...
    WYAMineCollectionDynamicModel * model = [[WYAMineCollectionDynamicModel alloc] init];
    model.userName                        = name;
    ...
    for (NSInteger index = 0; index < number; index++) {
        WYAMineCollectionDynamicImageModel * imageModel = [self aaa];
    ...
    for (NSInteger i = 0; i < comments; i++) {
        WYAMineCollectionDynamicCommentsModel * imageModel = [self ccc];
